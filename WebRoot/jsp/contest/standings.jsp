...
<head>
    <base href="<%=basePath%>">
    <title>Standings - <s:property value="contest.title"/> - HBUT Online Judge</title>
...
<div id="body">
    ...
    <div id="content">     <!-- class="content-with-sidebar" -->
            ...
            <div class="innertable" style="">
                ...
                <table class="standings">
                    <tr class="header">
                        ...
                        <s:if test="contest.type==0">
                            <th class="solved2">=</th>
                        </s:if>
                        <s:elseif test="contest.type==1">
                            <th class="score"><s:text name="score"/></th>
                        </s:elseif>
                        <th class="penalty"><s:text name="penalty"/></th>
                        <s:iterator value="cproblemList" status="st">
                            <th class="problem">
                                <span><a
                                        href="contest/<s:property value="contestId"/>/problem/<s:property value="num"/>"
                                        title="<s:property value="num"/> - <s:property value="title"/>"><s:property
                                        value="num"/></a></span>
                                <s:if test="contest.type==1"><br/><span style="font-weight: normal;"><s:property
                                        value="point"/>pt</span></s:if>
                            </th>
                        </s:iterator>
                    </tr>
                    <s:if test="attendList.size==0">
                        <tr>
                            <td class="left-item dark" colspan="32" style="text-align: left;">There is no records.</td>
                        </tr>
                    </s:if>
                    <s:iterator value="attendList" status="st">
                        <tr style="height: 42px;" username="<s:property value="username"/>"
                            class="<s:if test="#st.odd">dark</s:if> <s:if test="#session.session_username==username">my</s:if>">
                            <td class="rank left-item"><s:property value="rankList[#st.index]"/></td>
                            <td class="coder source">
                                <div style="float:left;">
                                    <b><a href="profile/<s:property value="username"/>" target="_blank" class="user-tip"
                                          user="<s:property value="username"/>"><s:property value="username"/></a></b>
                                </div>
                                <div style="float:right;font-size:12px;text-align:right;color: grey;">
                                    <span title="<s:property value="userList[#st.index].school" default=""/>"><s:property
                                            value="userList[#st.index].nickname" default=""/></span>
                                </div>
                            </td>
                            <s:if test="contest.type==0">
                                <td class="solved2"><span
                                        <s:if test="solved>0">style="color:#000;font-weight: bold;"</s:if>><s:property
                                        value="solved"/></span></td>
                            </s:if>
                            <s:elseif test="contest.type==1">
                                <td class="score"><span
                                        <s:if test="score>0">style="color:#000;font-weight: bold;"</s:if>><s:property
                                        value="score"/></span></td>
                            </s:elseif>
                            ...
                            <s:iterator value="acTimeList.get(username)" status="acst" var="actime">
                                ...
                                <s:if test="acTimeIntList.get(username)[#acst.index]>0">
                                    <td class="problem source"
                                        problemId="<s:property value="cproblemList[#acst.index].num" />">
                                        <b><span style="color:#32CD32">+<s:if
                                                test="wrongSubmits.get(username)[#acst.index]>0"><span
                                                title="<s:property value="wrongSubmits.get(username)[#acst.index]" /> wrong try"><s:property
                                                value="wrongSubmits.get(username)[#acst.index]"/></span></s:if></span></b>
                                    ...
                                    <s:if test="wrongSubmits.get(username)[#acst.index]>0">
                                        <td class="problem source"
                                            ...
                                            <span style="color:grey"
                                                  title="<s:property value="wrongSubmits.get(username)[#acst.index]" /> wrong try">-<s:property
                                                    value="wrongSubmits.get(username)[#acst.index]"/></span>
                                        </td>
                                    </s:if>
                                    <s:else>
                                        <td>&nbsp;</td>
        ...
        <div style="margin-right: 12px;">
            ...
            <div class="right">
                <s:if test="page>1">
                    <a href="contest/<s:property value="contestId"/>/standings/page/<s:property value="page-1"/>"
                       style="color:#000;text-decoration: none;">  &larr; </a>
                </s:if>
                <s:iterator value="pageList" status="st_page">
                    <a href="contest/<s:property value="contestId"/>/standings/page/<s:property/>"
    ...
    <script type="text/javascript"
            src="http://v3.jiathis.com/code/jiathis_r.js?uid=1339907478548202&type=left&amp;move=0"

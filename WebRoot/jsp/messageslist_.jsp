    ...
    <div id="content" class="content-with-sidebar ">
        ...
        <div class="content " style="position: relative;">
            <div class="comments">
                ...
                <s:if test="#st.index>0"></div>
            </s:if>
            <div class="roundbox-comments" style="position: relative;">
                </s:if>
                <s:else><s:if test="#st.index>0">
                    <hr>
                </s:if></s:else>
                <li style="">
                    <s:if test="nodeID[#st.index]!=''">
                    <div id=m_<s:property value="message_id"/> class="comment">
                        <img src="img/om.gif" alt="close it's comments" title="close it's comments" id=i
                            <s:property value="nodeID[#st.index]"/>
                             onClick="s2('<s:property value="nodeID[#st.index]"/>')">

                        <img src=<s:if test="message_id==root_id">"img/user.png"</s:if>
                        <s:else>"img/message-small.png"</s:else>/>

                        <s:if test="latestOrNot[#st.index]==1">
                            <span style="color:red;font-size:10px;">Latest~</span>
                        </s:if>
                        <b>Title:<span style="color:red;"><a href="topic/<s:property value="message_id"/>"><s:property
                                value="title"/></a></span> Posted By <b><a
                                href="profile/<s:property value="create_user"/>"><s:property
                                value="create_user"/></a></b> <span
                                title="<s:date name="in_date" nice="false" format="yyyy-MM-dd HH:mm:ss"/>">at <s:date
                                name="in_date" nice="true" format="yyyy-MM-dd HH:mm:ss"/></span>
                        </b>
                        <s:if test="#session.session_username==create_user"> | <a
                                href="topic/edit/<s:property value="message_id"/>">Eidt</a></s:if>
                        <br/>

                        <div class="content "><s:property value="content" escape="false"/></div>
                        <div>
                            <ul class=<s:property value="message_id"/>>
                                <li></li>
                            </ul>
                        </div>
                    </div>
                    <ul id=u<s:property value="nodeID[#st.index]"/>>
                        ...
                        <div id=m_<s:property value="message_id"/> class="comment">
                            <img src=<s:if test="message_id==root_id">"img/user.png"</s:if>
                            <s:else>"img/message-small.png"</s:else>/>
                            <s:if test="latestOrNot[#st.index]==1"><span
                                    style="color:red;font-size:10px;">Latest~</span></s:if>
                            <b>Title:<span style="color:red;"><a
                                    href="topic/<s:property value="message_id"/>"><s:property value="title"/></a></span>
                                Posted By <b><a href="profile/<s:property value="create_user"/>"><s:property
                                        value="create_user"/></a></b> <span
                                        title="<s:date name="in_date" nice="false" format="yyyy-MM-dd HH:mm:ss"/>">at <s:date
                                        name="in_date" nice="true" format="yyyy-MM-dd HH:mm:ss"/></span>
                            </b>
        ...
        <div style="margin-right: 12px;font-size:15px;">
            ...
            <div class="right">
                <s:if test="page>1">
                    <a href="topic/page/<s:property value="page-1"/>"

...
public class CalcRating {
	...
	public static final int PROVISIONAL = 250; // may consider 250 points as a
	...
	public static List<Double> expectancy(List<Integer> oldRating) {
		...
		for (int u = 0; u < oldRating.size(); u++) {
			...
			for (int o = 0; o < oldRating.size(); o++) {
				if (u != o) {
					ex += 1.0 / (1.0 + Math.pow(10.0,
							(oldRating.get(o) - oldRating.get(u)) / 400.0));
				}
	...
	public static List<Integer> initialize(List<Integer> contestTimes,
			List<Integer> oldRating) {
		...
		for (int i = 0; i < contestTimes.size(); i++) {
			if (contestTimes.get(i) < 1) {
				...
				oldRating_.add(oldRating.get(i));
		}
		return oldRating_;
	}
	 ...
	 * 
	 * 
	 * K = 30 (was 25) for a player new to the rating list until s/he has
	 * completed events with a total of at least 30 games.[15] K = 15 as long as
	 * a player's rating remains under 2200. K = 10 once a player's published
	 * rating has reached 2200, and s/he has also completed events with a total
	 * of at least 30 games. Thereafter it remains permanently at 10.
	 */
	// return grabs
	public static List<Integer> calc(List<Integer> contestTimes,
			List<Integer> oldRating, List<Integer> userRank) {
		List<Double> scores = new ArrayList<Double>();
		List<Double> probabilities = new ArrayList<Double>();

...
public class StreamHandler {
	...
	public static String read(String fileName) {
		/**
		 * read file by fileName, return a String pattern of file's context, or
		 * null if throw a IOException.
		 */
		try {
	...
	public static String read(File file) {
		/**
		 * read file by file, return a String pattern of file's context, or null
		 * if throw a IOException.
		 */
		try {
	...
	public static boolean write(String fileName, String content) {
		/**
		 * write content info file by fileName, return true if success, or false
		 * if throw a IOException.
		 */
		try {
	...
	public static boolean write(File file, String content) {
		/**
		 * write content info file by file, return true if success, or false if
		 * throw a IOException.
		 */
		try {
			PrintWriter out = new PrintWriter(new BufferedWriter(
	...
	public static List<File> getInputFiles(Integer problemId) {
		 ...
		 */
		File file = new File(Config.getValue("OJ_DATA_PATH")
				+ problemId.toString());
		...
		if (file.isDirectory()) {
			...
			List<File> filelist = new ArrayList<File>();
			for (int i = 0; i < files.length; i++) {
				String fileExt = files[i].getName().substring(
						files[i].getName().lastIndexOf(".") + 1,
						files[i].getName().length());
				// System.out.println(files[i].getName().lastIndexOf(".")+" "+fileExt+" "+
				// files[i].getName().length());

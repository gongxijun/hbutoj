...
public class ModifyUserAction extends ActionSupport {
    ...
    private final Logger logger = LoggerFactory.getLogger(ModifyUserAction.class);
    ...
    public String modifyUser() throws Exception {
        try {
			 ...
			 * 
			 * Matcher m =null; m = pEmail.matcher(user.getEmail()) ;
			 * if(m.matches()==false){ this.addFieldError("user.email",
			 * "Your email is invalid"); return INPUT; }
			 */
            ...
            if (null != user_) {
                ...
                if ((null == password || "".equals(password.trim()))
                        & (null == repeat_password || "".equals(repeat_password
                        .trim()))) {
                    // 不修改密码
					...
					/*
					 * System.out.println(password+" " +repeat_password); m =
					 * pPass.matcher(password); if(m.matches()==false){
					 * this.addFieldError("password",
					 * "password-length should be between 6 and 20"); return
					 * INPUT; }
					 */
                    if (!password.equals(repeat_password)) {
                        // 两次密码不一样
                        this.addFieldError("repeat_password",
                ...
                if (this.getUser().getMotto() != null
                        && this.getUser().getMotto().length() > 50) {

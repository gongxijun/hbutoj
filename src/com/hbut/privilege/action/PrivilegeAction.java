package com.hbut.privilege.action;
...
public class PrivilegeAction extends ActionSupport {
    ...
    private static final Logger logger = LoggerFactory.getLogger(PrivilegeAction.class);
    ...
    public String addPrivilege() {
        try {
            if ("".equals(privilege.getRightstr())
                    || privilege.getRightstr().length() < 1
                    || privilege.getRightstr().length() > 30) {
                return ERROR;
            }
            if (!(privilege.getRightstr().equals("ADMIN") || privilege
                    .getRightstr().equals("NOEDIT_ADMIN"))) {
            ...
            if (null != privilegeService
                    .query("from Privilege as p where p.username='"
                            + user_.getUsername() + "' and p.rightstr='"
                            + privilege.getRightstr() + "'")) {
                this.addFieldError("tip", "This user has the privilege("
                        + privilege.getRightstr() + ") now.");
                return ERROR;
            }
    ...
    public String deletePrivilege() {
        try {
            ...
            // TODO: handle exception
            logger.error("deletePrivilege: {}", e);
            return ERROR;
        }
        return SUCCESS;
    }

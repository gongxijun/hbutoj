...
public class JsonSolutionListAction extends ActionSupport {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JsonSolutionListAction.class);
    ...
    public String queryStatusList() throws Exception {
        try {
            ...
            sql_count = "select count(s.solution_id) from Solution s where s.contest_id="
                    + contestId
                    + " and s.submit_date between '"
                    + contest_.getStart_time()
                    + "' and '"
                    + contest_.getEnd_time() + "'";
            sql_query = "select s from Solution s where s.contest_id="
                    + contestId + " and s.submit_date between '"
                    + contest_.getStart_time() + "' and '"
                    + contest_.getEnd_time() + "'";

            if (problemId != null) {
                CProblem cproblem_ = new CProblem();
                cproblem_ = cproblemService.queryProblemByNum(problemId,
                        contestId);
                if (cproblem_ == null) {
                    success = false;
                ...
                sql_condition += " and s.problem_id="
                        + cproblem_.getProblem_id();
            }
            if (null != username) {
            ...
            for (Solution s : solutionList) {
                ...
                if (s.getVerdict() != 5 && s.getVerdict() < 12
                        && s.getVerdict() > 3) {
                ...
                // int 强制转换存在风险
                sb.setContestTimes(DateUtil.penaltyString((int) ((s
                        .getSubmit_date().getTime() - contest_.getStart_time()
                        .getTime()) / 1000)));

                CProblem cproblem_ = new CProblem();
                cproblem_ = cproblemService.queryProblemByPid(
                        s.getProblem_id(), contestId);
                if (cproblem_ != null) {

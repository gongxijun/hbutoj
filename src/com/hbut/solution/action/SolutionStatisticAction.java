...
public class SolutionStatisticAction extends ActionSupport {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SolutionStatisticAction.class);
    ...
    public String queryProblemStatistic() throws Exception {
        try {
            ...
            sql_count = "select count(s.solution_id) from Solution s where s.problem_id="
                    + problemId;

            submit = solutionService.countSolutions(sql_count);
            // sql_condition =
            // "select count(DISTINCT(s.username)) from Solution s where s.verdict=5 and s.problem_id="+problemId;
            // solved = solutionService.countSolutions(sql_count+sql_condition);

            countList = new ArrayList<Integer>();

            sql_condition = " and s.verdict=5";
            countList.add(solutionService.countSolutions(sql_count
                    + sql_condition));
            ...
            logger.error("queryProblemStatistic: {}", e);
            return ERROR;
        }
        return SUCCESS;
    }

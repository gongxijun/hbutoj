...
public class MessageDaoImpl extends HibernateDaoSupport implements MessageDAO {
    ...
    public List<Message> query(String sql) {
        ...
        return getHibernateTemplate()
                .find("from Message as m order by m.orderNum ASC, m.root_id DESC,m.parent_id ASC,m.message_id ASC");
    }

    public List<Message> queryRootMessages(Integer pageNow, Integer pageSize) {
        /*
		 * String sql =
		 * "from Message as m group by m.root_id  order by max(m.message_id) DESC"
		 * ; HibernateTemplate ht = this.getHibernateTemplate();
		 * ht.setMaxResults(limit); List<Message> list = ht.find(sql); return
		 * list;
		 */
        ...
        // 以楼主时间排序
        // String sql =
        // "from Message as m where m.message_id=m.root_id and m.defunct='N' order by m.orderNum ASC,m.message_id DESC";
    ...
    public Message getLatestReply(Integer messageId) {
        ...
        session.beginTransaction();
        // 以最新评论或发表为序
        String sql = "from Message m where m.parent_id!=0 and m.root_id="
                + messageId + " order by m.message_id DESC";
    ...
    public List<Message> queryLatestMessages(String username, Integer pageNow,
        ...
        if (username == null) {
            ...
            sql = "from Message as m where m.create_user='" + username
                    + "' and m.defunct='N' order by m.message_id DESC";
        }
        Query q = (Query) session.createQuery(sql);
        q.setFirstResult(from);

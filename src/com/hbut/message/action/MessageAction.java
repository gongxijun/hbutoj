...
public class MessageAction extends ActionSupport {
    ...
    public String execute() throws Exception {
        ...
        while (messageQueue_.size() != 0) {
            ...
            messageList__ = messageService.queryChildMessages(messageQueue_
                    .get(size_ - 1).getMessage_id());
        ...
        for (Integer i = 1; i < size; i++) {
            ...
            if (messageList.get(i).getMessage_id()
                    .equals(messageList.get(i).getRoot_id())) {
                ...
                if (messageList.get(i).getParent_id() > messageList.get(i - 1)
                        .getParent_id()) {
                    nodeUL.add("");
                    nodeID_.add(a + "_" + b);
                    b++;
                ...
                } else if (messageList.get(i).getParent_id() < messageList.get(
                        i - 1).getParent_id()) {

                    int j = 0;
                    for (j = i - 1; j >= 0; j--) {
                        if (messageList.get(i).getParent_id()
                                .equals(messageList.get(j).getParent_id())) {
                            // System.out.println(nodeULPre.get(i-1)+" | "+nodeULPre.get(j));
                            nLeftUL = nLeftUL
                                    - (nodeULPre.get(i - 1) - nodeULPre.get(j));
                            nodeULPre.add(nLeftUL);

                            String strUL__ = new String();
                            for (int ii = 0; ii < nodeULPre.get(i - 1)
                                    - nodeULPre.get(j); ii++) {
                                strUL__ += "</UL>";
                            }
                            nodeUL.add(strUL__);
    ...
    public String postMessage() throws Exception {
        ...
        try {
            ...
            createUser = (String) ActionContext.getContext().getSession()
                    .get("session_username");
            System.out.println(moduleId + " " + parentId + " " + rootId + " "
                    + createUser);
    ...
    public String showMessage() throws Exception {
        ...
        try {
            ...
            if (message_ == null
                    || !message_.getRoot_id().equals(message_.getMessage_id())) {
            ...
            // System.out.println(messageId+" "+message_.getTitle()+" "+
            // message_.getContent());
            if (message_.getParent_id() > 0) {
                Message messageReplyTo_ = messageService.queryMessage(message_
                        .getParent_id());
            ...
            for (Integer i = 0; i < size; i++) {
                ...
                if (messageList.get(i).getMessage_id()
                    ...
                    if (messageList.get(i).getParent_id() > messageList.get(
                            i - 1).getParent_id()) {
                        nodeUL.add("");
                        nodeID_.add(a + "_" + b);
                        b++;
                        nLeftUL++;
                        nodeULPre.add(nLeftUL);
                    } else if (messageList.get(i).getParent_id() < messageList
                            .get(i - 1).getParent_id()) {
            ...
            messagebean
                    .setFriendly_Date(getFriendlyDate(message_.getIn_date()));
            messagebean.setViews(message_.getViews());
            messagebean.setVotes(message_.getVotes());

            String bufString = new String();
            bufString = Html2Text
                    .RemoveHtml(((message_.getContent().length() > 2000) ? (message_
                            .getContent()).substring(0, 2000) : message_
                            .getContent()));

            messagebean
                    .setContent_abstract(((bufString.length() > 200) ? (bufString)
                            .substring(0, 200) + "..."
                            : bufString));

        } catch (Exception e) {
            // TODO: handle exception
            logger.error("showMessage : {}", e);
            return ERROR;
        }
        return SUCCESS;
    }

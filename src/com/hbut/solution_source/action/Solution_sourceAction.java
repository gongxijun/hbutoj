...
public class Solution_sourceAction extends ActionSupport {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Solution_sourceAction.class);
    ...
    public String solutionSource() throws Exception {
        ...
        try {
            System.out
                    .println("Source[" + solutionId + "] is open via link...");
                ...
                if (!username.equals(solution_.getUsername())) {
                    if ("NO".equals(Config.getValue("OPENSOURCE"))) {
                        ActionContext
                                .getContext()
                                .put("tip",
                                        "This operation is now closed by Administrator.");
                        return ERROR;
                    }
                    if (user_.getOpensource().equals("N")) {
                        ActionContext.getContext().put(
                                "tip",
                                "This source doesn't open for you.You can write mail to "

...
public class UserInfoTipAction extends ActionSupport {
    ...
    public String userInfo() throws IOException {
        ...
        String sql = "select count(m.message_id) from Message m where "
                + "m.defunct='N' and m.parent_id=0 and m.create_user='"
                + user_.getUsername() + "'";
        ...
        if (user_.getAvatar() != null && user_.getAvatar().equals("Y")) {
            buf += "<img src='upload/userphoto/" + user_.getUsername()
                    + "/photo_80.jpg' alt='" + user_.getUsername() + "'>";
        } else {
        ...
        buf += "<h1><a href='mails/new/" + user_.getUsername()
                + "' title='Mail to " + user_.getUsername() + "'>@"
                + user_.getUsername() + "</a></h1><span class='city'>";
        if (user_.getSchool() != null && !user_.getSchool().trim().equals("")) {

            buf += (user_.getSchool().length() > 16) ? user_.getSchool()
                    .subSequence(0, 15) + "..." : user_.getSchool();
        }
        buf += "</span>";

        buf += "<div id='UserMotto'>";
        if (user_.getMotto() != null && !user_.getMotto().trim().equals("")) {
            buf += (user_.getMotto().length() > 30) ? user_.getMotto()
                    .subSequence(0, 30) + "..." : user_.getMotto();
        }
        buf += "</div><div style='clear:both'></div>";
        buf += "<div id='UserStat'>" + "<ul>" + "<li><em>"
                + userService.getUserRank(user_) + "</em>" + getText("rank")
                + "</li>" + "<li><a href='problemset/status/with/"
                + user_.getUsername() + "/page/1'><em>" + user_.getSolved()
                + "</em>" + getText("solved") + "</a></li>"
                + "<li><a href='problemset/status/with/" + user_.getUsername()
                + "/page/1'><em>" + user_.getSubmit() + "</em>"
                + getText("submit") + "</a></li>"
                + "<li style='border-right:0;'><a href='#'><em>" + nCountTopic
                + "</em>" + getText("topic") + "</a></li>" + "</ul>" + "</div>"
                +
        ...
        if (messages_ != null) {
            if (messages_.size() == 0) {
                buf += "<span style='color:grey'>" + getText("no-any-action")
                        + "</span>";
            }
            for (Message m : messages_) {
                String bufString = new String();
                bufString = Html2Text
                        .RemoveHtml(((m.getContent().length() > 500) ? (m
                                .getContent()).substring(0, 500) : m
                                .getContent()));
                String url = "";
                String img = "";
                if (m.getParent_id() > 0) {
                    url = "topic/" + m.getRoot_id() + "#rpl_"
                            + m.getMessage_id();
                ...
                buf += img
                        ...
                        + ((bufString.length() > 100) ? (bufString).substring(
                        0, 100) + "..." : bufString) + "</span></br>";
